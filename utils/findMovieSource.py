...
def detail_page(positions):
    ...
    # 线程执行的方法名称process_detail_page  线程所需要的集合参数movie_url_list
    movie = pool.map(process_detail_page, movie_url_list)
...
def parse_detail_page(detailPage):
    ...
    movie_name = tree.xpath('//div[@class="vodh"]/h2/text()')
    # 电影观看url 和 下载url
